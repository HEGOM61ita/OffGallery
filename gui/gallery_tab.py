...
class ViewportXMPManager:
    ...
    def enable_viewport_checking(self):
        ...
        if hasattr(self.gallery, 'scroll_area'):
            ...
            print("❌ DEBUG: scroll_area non trovato")
    ...
    def __init__(self, gallery_tab):
        ...
        # TEST DEBUG
        print(f"🔧 DEBUG: ViewportXMPManager inizializzato per gallery: {gallery_tab}")
    ...
    def _force_initial_check(self):
        """Test forzato per verificare che il threading funzioni"""
        print("🧪 DEBUG: _force_initial_check attivato")
        try:
            if hasattr(self.gallery, 'cards') and self.gallery.cards:
                print(f"🧪 DEBUG: Trovate {len(self.gallery.cards)} cards")
                test_cards = self.gallery.cards[:3]  # Prime 3
                self.xmp_checker.request_xmp_check(test_cards)
            else:
                print("❌ DEBUG: Nessuna card trovata")
        except Exception as e:
            print(f"❌ DEBUG: Errore force check: {e}")
...
class GalleryTab(QWidget):
    ...
    def __init__(self, parent=None, ai_models=None):
        ...
        # TEST DEBUG
        print(f"🔧 DEBUG: viewport_xmp_manager creato: {self.viewport_xmp_manager}")
        self.init_ui()
    ...
    def display_results(self, results):
        ...
        count = len(results)
        print(f"🔍 DEBUG: Iniziando display di {count} risultati...")
        import time
        ...
        if count == 0:
            ...
            print(f"⏱️ DEBUG: Display completato in {elapsed:.2f} secondi")
            return
    ...
    def _reload_sync_states_from_db(self):
        ...
        try:
            ...
            db_manager.close()
            print(f"DEBUG: Ricaricati sync_state dal DB per {len(self.current_results)} immagini")
    ...
    def _refresh_gallery_sync_states(self):
        ...
        try:
            ...
            print(f"Aggiornate {len(self.cards)} card con nuovi sync_state")
        
        except Exception as e:
            print(f"Errore refresh gallery sync states: {e}")
            import traceback
            traceback.print_exc()
            print(f"Errore refresh sync states: {e}")
    ...
    def find_similar(self, item):
        ...
        try:
            ...
            print(f"🔧 DEBUG: threshold = {threshold}, max_results = {max_results}")
            ...
            print(f"🔧 DEBUG: ref_embedding blob size = {len(row[0]) if row[0] else 'None'}")
            print(f"🔧 DEBUG: ref_embedding blob type = {type(row[0])}")
        
            try:
                # Prova pickle prima
                ref_embedding = pickle.loads(row[0])
                print(f"🔧 DEBUG: Deserializzato con pickle: shape={ref_embedding.shape}")
            except:
                try:
                    # Se pickle fallisce, prova frombuffer
                    ref_embedding = np.frombuffer(row[0], dtype=np.float32)
                    print(f"🔧 DEBUG: Deserializzato con frombuffer: shape={ref_embedding.shape}")
                except Exception as e:
                    print(f"🔧 DEBUG: Errore deserializzazione: {e}")
                    progress.close()
                    db_manager.close()
                    return
            ...
            for i, row in enumerate(all_rows):
                ...
                try:
                    ...
                    print(f"🔧 DEBUG: similarity = {similarity:.3f} vs threshold = {threshold}")
                    
                    if similarity >= threshold:
                        image_data['similarity_score'] = similarity
                        results.append(image_data)
                except Exception as e:
                    print(f"🔧 DEBUG: Errore deserializzazione embedding: {e}")
                    continue
    ...
    def reimport_image_metadata(self, image_id, filepath):
        ...
        try:
            ...
            print(f"Reimporting metadata da {filepath.name} usando sistema XMP...")
            ...
            if hasattr(target_card, 'xmp_state') and target_card.xmp_state:
                if 'OUT' in str(target_card.xmp_state) or 'DIRTY' in str(target_card.xmp_state):
                    ...
                    print(f"📊 Nessuna modifica XMP rilevata per {filepath.name}")
                    if self.parent_window:
