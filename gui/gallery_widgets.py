...
class ImageCard(QFrame):
    ...
    def _load_thumbnail(self):
        ...
        try:
            if not self.filepath or not self.filepath.exists():
                ...
                self.thumbnail_label.setStyleSheet(self.thumbnail_label.styleSheet() + f"color: {COLORS['grigio_medio']};")
                return
    ...
    def get_unified_tags(self):
        ...
        try:
            ...
            print(f"🔍 DEBUG get_unified_tags: {self.image_data.get('filename', 'Unknown')}")
            print(f"  Raw tags from DB: {repr(tags_raw)} (type: {type(tags_raw)})")
            
            if not tags_raw:
                self._unified_tags_cache = []
                print(f"  Result: Empty (no tags_raw)")
                return self._unified_tags_cache
            
            if isinstance(tags_raw, str):
                try:
                    tags = json.loads(tags_raw)
                    print(f"  Parsed JSON: {tags} (type: {type(tags)})")
                ...
                tags = tags_raw
                print(f"  Direct tags: {tags} (type: {type(tags)})")
                
            self._unified_tags_cache = tags if isinstance(tags, list) else []
            
            print(f"  Final result: {self._unified_tags_cache} ({len(self._unified_tags_cache)} items)")
            return self._unified_tags_cache
    ...
    def _build_semantic_tooltip(self):
        ...
        try:
            ...
            # Rating e Color Label (se presenti)
            rating = self.image_data.get('lr_rating') or self.image_data.get('rating')
            color_label = self.image_data.get('color_label')
            rating_color_line = []
            if rating and 1 <= int(rating) <= 5:
                rating_color_line.append(f"⭐ {'★' * int(rating)}")
            if color_label:
                rating_color_line.append(f"🎨 {color_label}")
            if rating_color_line:
                lines.append(" | ".join(rating_color_line))
            ...
            # FIX: Safe print con controlli None
            tag_count = len(unified_tags) if unified_tags else 0
            desc_length = len(description) if description else 0
            print(f"🔄 Cache semantic tooltip rebuild per {filename}: {tag_count} tag, {desc_length} char descrizione")
    ...
    def _open_folder(self, items):
        """Apri cartella contenente le immagini"""
        try:
            folders = set()
            for item in items:
    ...
    def _open_with_external_editor(self, editor):
        ...
        try:
            ...
            try:
                ...
                print(f"🎨 Avvio editor: {' '.join(command)}")
    ...
    def is_selected(self):
        ...
        print(f"📝 Monitoraggio processo {editor_name} avviato...")
    ...
    def _on_editor_closed(self, has_changes, editor_name):
        ...
        print(f"🎨 Editor {editor_name} chiuso. Modifiche rilevate: {has_changes}")
        
        if has_changes:
            # Reimport automatico senza popup
            print("🔄 Avvio reimport automatico...")
    ...
    def _edit_tags(self, items):
        ...
        try:
            ...
            if dialog.exec() == QDialog.DialogCode.Accepted:
                ...
                print(f"Salvando {len(new_tags)} tag su database per {len(items)} elementi: {new_tags}")
                ...
                for item in items:
                    if hasattr(item, 'image_id') and item.image_id:
                        try:
                            ...
                            print(f"✅ Tag salvati su DB: {item.image_data.get('filename')} - {new_tags}")
                            db_success += 1
                        except Exception as e:
                            print(f"❌ Errore scrittura tag DB per {item.image_data.get('filename')}: {e}")
                            db_failures += 1
                    else:
                        print(f"❌ Nessun image_id per {item.image_data.get('filename')}")
                        db_failures += 1
                ...
                print(f"📊 RISULTATO: {db_success} successi, {db_failures} fallimenti")
        except Exception as e:
    ...
    def _edit_description(self, items):
        ...
        try:
            ...
            if dialog.exec() == QDialog.DialogCode.Accepted:
                ...
                print(f"Salvando descrizione su database per {len(items)} elementi: {new_description}")
                ...
                for item in items:
                    if hasattr(item, 'image_id') and item.image_id:
                        try:
                            ...
                            print(f"✅ Descrizione salvata su DB: {item.image_data.get('filename')} - {new_description[:50]}...")
                            db_success += 1
                        except Exception as e:
    ...
    def _edit_title(self, items):
        ...
        try:
            ...
            if dialog.exec() == QDialog.DialogCode.Accepted:
                ...
                print(f"Salvando titolo su database per {len(items)} elementi: {new_title}")
                ...
                for item in items:
                    if hasattr(item, 'image_id') and item.image_id:
                        try:
                            ...
                            print(f"✅ Titolo salvato su DB: {item.image_data.get('filename')} - {new_title}")
                            db_success += 1
                        except Exception as e:
    ...
    def _set_rating(self, items, rating_value):
        ...
        try:
            print(f"Impostando rating {rating_value} per {len(items)} elementi")
            ...
            for item in items:
                if hasattr(item, 'image_id') and item.image_id:
                    try:
                        ...
                        stars_display = '★' * rating_value if rating_value > 0 else 'nessuno'
                        print(f"✅ Rating impostato: {item.image_data.get('filename')} → {stars_display}")
                        db_success += 1
            ...
            print(f"📊 Rating: {db_success} successi, {db_failures} fallimenti")
    ...
    def _set_color_label(self, items, color_value):
        ...
        try:
            print(f"Impostando color label '{color_value}' per {len(items)} elementi")
            ...
            for item in items:
                if hasattr(item, 'image_id') and item.image_id:
                    try:
                        ...
                        color_display = color_value if color_value else 'nessuno'
                        print(f"✅ Color label impostato: {item.image_data.get('filename')} → {color_display}")
                        db_success += 1
            ...
            print(f"📊 Color Label: {db_success} successi, {db_failures} fallimenti")
    ...
    def _delete_from_database(self, items):
        ...
        try:
            ...
            # Messaggio risultato
            if deleted > 0:
                print(f"🗑️ Eliminate {deleted} immagini dal database")
                if failed > 0:
                    QMessageBox.warning(
                        self,
    ...
    def _run_bioclip_and_refresh(self, items):
        ...
        try:
            ...
            # Refresh automatico sarà gestito dal parent dopo completamento
            print(f"BioCLIP avviato per {len(items)} elementi")
        except Exception as e:
    ...
    def _run_llm_and_refresh(self, items):
        ...
        try:
            ...
            # Refresh automatico sarà gestito dal parent dopo completamento
            print(f"LLM tagging avviato per {len(items)} elementi")
        except Exception as e:
                ...
                for item in items:
                    if hasattr(item, 'filepath') and item.filepath:
                        ...
                        if fresh_data:
                            ...
                            print(f"🔄 Dati ricaricati dal DB per {item.image_data.get('filename', 'Unknown')}")
                            print(f"   Title: {fresh_data.get('title', '(vuoto)')}")
    ...
    def _force_data_refresh(self):
        ...
        try:
            ...
            print(f"🔄 Cache dati invalidata per {self.image_data.get('filename', 'Unknown')}")
    ...
    def _refresh_badges(self):
        ...
        try:
            if hasattr(self, 'info_frame') and self.info_frame.layout():
                print(f"🔄 Refresh badge per {self.image_data.get('filename', 'Unknown')}")
    ...
    def _invalidate_tooltip_cache(self):
        ...
        try:
            ...
            print(f"🔄 Cache tooltip invalidata per {self.image_data.get('filename', 'Unknown')}")
    ...
    def _analyze_xmp_detailed(self, items):
        ...
        try:
            ...
            for item in items:
                ...
                print(f"🔄 XMP Analysis (3-way): {filepath.name}")
                ...
                if sidecar_exists:
                    ...
                    if sidecar_data:
                        ...
                        print(f"✅ Sidecar: title='{sidecar_title}', desc={len(sidecar_desc)}c, tags={len(sidecar_tags)}")
                ...
                if embedded_supported:
                    ...
                    if embedded_data:
                        ...
                        print(f"✅ Embedded: title='{embedded_title}', desc={len(embedded_desc)}c, tags={len(embedded_tags)}")
    ...
    def _import_from_xmp_with_refresh(self, items):
        ...
        # Setup file logging per debug
        from pathlib import Path
        from datetime import datetime
        log_file = get_app_dir() / 'xmp_import_debug.log'
        ...
        try:
            with open(log_file, 'w', encoding='utf-8') as f:
                f.write(f"=== XMP Import Debug Log - {datetime.now()} ===\n\n")
        except:
            pass

        # GUARD: Previeni chiamate multiple
        if hasattr(self, '_importing_xmp') and self._importing_xmp:
            log_debug("🚨 DEBUG: Import XMP già in corso, ignoro richiesta duplicata")
            return

        try:
            self._importing_xmp = True  # Flag per prevenire chiamate multiple
            log_debug(f"🔍 DEBUG: Avvio import XMP per {len(items)} elementi")
            ...
            if reply != QMessageBox.StandardButton.Yes:
                print("🔍 DEBUG: Utente ha annullato import")
                return
            
            # Check database availability
            print("🔍 DEBUG: Controllo disponibilità DatabaseManager...")
            db_manager = self._get_database_manager()
            if not db_manager:
                print("🔍 DEBUG: DatabaseManager NON disponibile")
                self._show_xmp_dialog("❌ Errore", "DatabaseManager non disponibile - operazione annullata")
                return
            else:
                print("🔍 DEBUG: DatabaseManager OK")
            ...
            for i, item in enumerate(items):
                print(f"\n🔍 DEBUG: Elaborando elemento {i+1}/{len(items)}")
                
                try:
                    # Check 1: item ha filepath?
                    if not hasattr(item, 'filepath'):
                        print(f"❌ DEBUG: Elemento non ha attributo 'filepath'")
                        error_count += 1
                        continue
                    
                    if not item.filepath:
                        print(f"❌ DEBUG: filepath è None")
                        error_count += 1
                        continue
                    ...
                    # Check 2: file esiste?
                    if not filepath.exists():
                        print(f"❌ DEBUG: File NON ESISTE: {filepath}")
                        error_count += 1
                        continue
                    else:
                        print(f"✅ DEBUG: File esiste: {filepath}")
                    
                    # Check 3: item ha image_id?
                    if not hasattr(item, 'image_id'):
                        print(f"❌ DEBUG: Elemento non ha attributo 'image_id'")
                        print(f"   Attributi disponibili: {[attr for attr in dir(item) if not attr.startswith('_')]}")
                        error_count += 1
                        continue
                    
                    if not item.image_id:
                        print(f"❌ DEBUG: image_id è None o vuoto: {repr(item.image_id)}")
                        error_count += 1
                        continue
                    else:
                        print(f"✅ DEBUG: image_id = {item.image_id}")
                    
                    # Check 4: XMP source disponibile?
                    sidecar_path = filepath.with_suffix('.xmp')
                    xmp_source = None
                    
                    print(f"🔍 DEBUG: Controllo sidecar: {sidecar_path}")
                    if sidecar_path.exists():
                        xmp_source = sidecar_path
                        print(f"✅ DEBUG: Trovato sidecar XMP: {sidecar_path}")
                    else:
                        print(f"⚠️ DEBUG: Sidecar NON trovato: {sidecar_path}")
                        
                        # Check embedded XMP
                        embedded_formats = {'.jpg', '.jpeg', '.tif', '.tiff', '.dng', '.cr2', '.nef', '.orf', '.arw', '.rw2', '.pef', '.cr3', '.nrw', '.srf', '.sr2'}
                        print(f"🔍 DEBUG: File extension: {filepath.suffix.lower()}")
                        if filepath.suffix.lower() in embedded_formats:
                            xmp_source = filepath
                            print(f"✅ DEBUG: Usando embedded XMP: {filepath}")
                        else:
                            print(f"❌ DEBUG: Formato non supporta embedded XMP: {filepath.suffix}")
                    
                    if not xmp_source:
                        print(f"❌ DEBUG: Nessun XMP disponibile per {filepath.name}")
                        no_xmp_count += 1
                        continue
                    
                    # Check 5: XMP leggibile?
                    print(f"🔍 DEBUG: Lettura XMP da: {xmp_source}")
                    xmp_data = self._read_xmp_with_exiftool(xmp_source)
                    if not xmp_data:
                        print(f"❌ DEBUG: XMP vuoto o errore lettura per {filepath.name}")
                        no_xmp_count += 1
                        continue
                    else:
                        print(f"✅ DEBUG: XMP data keys: {list(xmp_data.keys())}")
                    
                    # Check 6: Estrai dati XMP
                    new_tags = []
                    keyword_fields = ['Keywords', 'Subject', 'HierarchicalSubject']
                    
                    print(f"🔍 DEBUG: Ricerca keywords in campi: {keyword_fields}")
                    for field in keyword_fields:
                        if field in xmp_data and xmp_data[field] is not None:
                            value = xmp_data[field]
                            print(f"✅ DEBUG: Trovato {field}: {repr(value)} (type: {type(value)})")
                            ...
                            print(f"⚠️ DEBUG: Campo {field} non trovato o None")
                    
                    # Estrai descrizione
                    new_description = ""
                    desc_fields = ['Description', 'Caption-Abstract', 'ImageDescription']
                    print(f"🔍 DEBUG: Ricerca descrizione in campi: {desc_fields}")
                    
                    for desc_field in desc_fields:
                        if desc_field in xmp_data and xmp_data[desc_field] is not None:
                            desc_value = xmp_data[desc_field]
                            print(f"✅ DEBUG: Trovato {desc_field}: {repr(desc_value)[:100]}...")
                            if desc_value is not None:
                                new_description = str(desc_value).strip()
                                if new_description:
                                    break
                        else:
                            print(f"⚠️ DEBUG: Campo {desc_field} non trovato o None")
                    
                    print(f"🔍 DEBUG: Risultato parsing - Tags: {len(new_tags)}, Description: {len(new_description)} char")
                    print(f"   Tags: {new_tags}")
                    print(f"   Desc: {new_description[:100]}...")
                    
                    # Check 7: Hai dati da importare?
                    if not new_tags and not new_description:
                        print(f"⚠️ DEBUG: Nessun dato da importare per {filepath.name}")
                        no_xmp_count += 1
                        continue
                    
                    # Check 8: Scrivi su database
                    # IMPORTANTE: Verifica metodi disponibili in DatabaseManager
                    print(f"🔍 DEBUG: Metodi DatabaseManager disponibili:")
                    db_methods = [method for method in dir(db_manager) if not method.startswith('_')]
                    print(f"   {db_methods}")
                    
                    db_updated = False
                    
                    if new_tags:
                        print(f"🔍 DEBUG: Scrittura {len(new_tags)} tag su DB...")
                        try:
                            # Prova diversi possibili nomi di metodi
                            if hasattr(db_manager, 'update_tags'):
                                print(f"   Usando: db_manager.update_tags()")
                                result = db_manager.update_tags(item.image_id, new_tags)
                            elif hasattr(db_manager, 'update_image_tags'):
                                print(f"   Usando: db_manager.update_image_tags()")
                                result = db_manager.update_image_tags(item.image_id, new_tags)
                            elif hasattr(db_manager, 'set_tags'):
                                print(f"   Usando: db_manager.set_tags()")
                                result = db_manager.set_tags(item.image_id, new_tags)
                            elif hasattr(db_manager, 'update_metadata'):
                                print(f"   Usando: db_manager.update_metadata() per tag")
                                result = db_manager.update_metadata(item.image_id, tags=new_tags)
                            else:
                                print(f"❌ DEBUG: Nessun metodo trovato per aggiornare tag!")
                                print(f"   Metodi che iniziano con 'update': {[m for m in db_methods if m.startswith('update')]}")
                                print(f"   Metodi che contengono 'tag': {[m for m in db_methods if 'tag' in m.lower()]}")
                                continue
                                
                            print(f"   Risultato: {result}")
                            item.image_data['tags'] = json.dumps(new_tags)
                            print(f"✅ DEBUG: Tag salvati su DB per {filepath.name}")
                            db_updated = True
                        except Exception as e:
                            print(f"❌ DEBUG: Errore scrittura tag su DB: {e}")
                            import traceback
                            traceback.print_exc()
                    
                    if new_description:
                        print(f"🔍 DEBUG: Scrittura descrizione su DB...")
                        try:
                            # Prova diversi possibili nomi di metodi
                            if hasattr(db_manager, 'update_description'):
                                print(f"   Usando: db_manager.update_description()")
                                result = db_manager.update_description(item.image_id, new_description)
                            elif hasattr(db_manager, 'update_image_description'):
                                print(f"   Usando: db_manager.update_image_description()")
                                result = db_manager.update_image_description(item.image_id, new_description)
                            elif hasattr(db_manager, 'set_description'):
                                print(f"   Usando: db_manager.set_description()")
                                result = db_manager.set_description(item.image_id, new_description)
                            elif hasattr(db_manager, 'update_metadata'):
                                print(f"   Usando: db_manager.update_metadata() per description")
                                result = db_manager.update_metadata(item.image_id, description=new_description)
                            else:
                                print(f"❌ DEBUG: Nessun metodo trovato per aggiornare descrizione!")
                                print(f"   Metodi che contengono 'desc': {[m for m in db_methods if 'desc' in m.lower()]}")
                                continue
                                
                            print(f"   Risultato: {result}")
                            item.image_data['description'] = new_description
                            print(f"✅ DEBUG: Descrizione salvata su DB per {filepath.name}")
                            db_updated = True
                        except Exception as e:
                            print(f"❌ DEBUG: Errore scrittura descrizione su DB: {e}")
                            import traceback
                            traceback.print_exc()
                    # Estrai e aggiorna title se presente
                    new_title = ""
                    title_fields = ['Title', 'XMP-dc:Title', 'XMP:Title', 'ObjectName', 'Headline']
                    log_debug(f"🔍 TITLE DEBUG: Cerco title in campi: {title_fields}")
                    log_debug(f"🔍 TITLE DEBUG: xmp_data keys disponibili: {list(xmp_data.keys())}")
                    for title_field in title_fields:
                        if title_field in xmp_data and xmp_data[title_field]:
                            ...
                            if new_title:
                                log_debug(f"✅ TITLE DEBUG: Trovato title in campo '{title_field}': {new_title}")
                                break

                    if new_title:
                        try:
                            log_debug(f"🔍 TITLE DEBUG: Tentativo update title per image_id={item.image_id}, title='{new_title}'")
                            title_updated = False
                            if hasattr(db_manager, 'update_title'):
                                log_debug(f"🔍 TITLE DEBUG: Chiamo db_manager.update_title({item.image_id}, '{new_title}')")
                                title_updated = db_manager.update_title(item.image_id, new_title)
                                log_debug(f"🔍 TITLE DEBUG: update_title returned: {title_updated}")
                            elif hasattr(db_manager, 'update_image_title'):
                                log_debug(f"🔍 TITLE DEBUG: Chiamo db_manager.update_image_title({item.image_id}, '{new_title}')")
                                title_updated = db_manager.update_image_title(item.image_id, new_title)
                                log_debug(f"🔍 TITLE DEBUG: update_image_title returned: {title_updated}")
                            else:
                                log_debug(f"❌ TITLE DEBUG: Nessun metodo per aggiornare title!")

                            if title_updated:
                                item.image_data['title'] = new_title
                                log_debug(f"✅ TITLE DEBUG: Title salvato su DB: {new_title}")
                                db_updated = True
                            else:
                                log_debug(f"⚠️ TITLE DEBUG: Title NON salvato - update ha ritornato False")
                        except Exception as e:
                            print(f"⚠️ Errore aggiornamento title: {e}")
                            import traceback
                            traceback.print_exc()
                    if db_updated:
                        success_count += 1
                        print(f"✅ DEBUG: Import completato per {filepath.name}")
                    
                except Exception as e:
                    print(f"❌ DEBUG: Errore generale per elemento {i+1}: {e}")
                    import traceback
                    traceback.print_exc()
                    error_count += 1
            ...
            print(f"❌ DEBUG: Errore globale import XMP: {e}")
            import traceback
            traceback.print_exc()
            ...
            print(f"🔍 DEBUG: Import XMP terminato")
    ...
    def _export_to_xmp_with_refresh(self, items):
        ...
        if hasattr(self, '_exporting_xmp') and self._exporting_xmp:
            print("🚨 DEBUG: Export XMP già in corso, ignoro richiesta duplicata")
            return

        try:
            self._exporting_xmp = True  # Flag per prevenire chiamate multiple
            print(f"📤 DEBUG: Avvio export XMP per {len(items)} elementi")
            ...
            if reply != QMessageBox.StandardButton.Yes:
                print("📤 DEBUG: Utente ha annullato export")
                return
            ...
            try:
                ...
                print(f"❌ DEBUG: Errore caricamento XMPManagerExtended: {e}")
            ...
            for i, item in enumerate(items):
                ...
                try:
                    # Check filepath
                    if not hasattr(item, 'filepath') or not item.filepath:
                        print(f"❌ DEBUG: Elemento senza filepath")
                        error_count += 1
                        continue
                    ...
                    print(f"📤 DEBUG: Dati da esportare:")
                    print(f"   Title: {title}")
                    print(f"   Description: {description[:50]}..." if description else "   Description: (vuota)")
                    print(f"   Tags: {tags}")

                    # Verifica se ci sono dati da esportare
                    if not title and not description and not tags:
                        print(f"⚠️ DEBUG: Nessun dato da esportare per {filepath.name}")
                        skipped_count += 1
                        continue
                    ...
                    print(f"📤 DEBUG: Scrittura XMP per {filepath.name}...")

                    # Scrivi XMP usando il manager
                    success = xmp_manager.write_xmp_by_format(filepath, xmp_dict)

                    if success:
                        print(f"✅ DEBUG: XMP scritto con successo per {filepath.name}")
                        success_count += 1
                    else:
                        print(f"❌ DEBUG: Errore scrittura XMP per {filepath.name}")
                        error_count += 1
            ...
            print(f"❌ DEBUG: Errore globale export XMP: {e}")
            import traceback
            traceback.print_exc()
            ...
            print(f"📤 DEBUG: Export XMP terminato")
            ...
            for item in items:
                try:
                    ...
                    if new_tags:
                        item.image_data['tags'] = json.dumps(new_tags)
                        updated = True
                        print(f"🔄 TEMP SIMULATION TAGS: {filepath.name} - {new_tags}")
                    
                    if new_description:
                        item.image_data['description'] = new_description
                        updated = True
                        print(f"🔄 TEMP SIMULATION DESC: {filepath.name} - {new_description[:50]}...")
    ...
    def _show_xmp_content(self, items):
        ...
        try:
            ...
            print(f"🔍 DEBUG XMP Keywords: Found {len(keywords)} keywords: {keywords}")
    ...
    def _get_database_manager(self):
        ...
        try:
            print("🔍 DEBUG: Import DatabaseManager...")
            from db_manager_new import DatabaseManager
            print("✅ DEBUG: DatabaseManager importato")
            
            print("🔍 DEBUG: Ricerca config_new.yaml...")
            import yaml
            from pathlib import Path
            ...
            for config_path in config_paths:
                print(f"🔍 DEBUG: Controllo: {config_path}")
                
                if config_path.exists():
                    print(f"✅ DEBUG: config_new.yaml TROVATO: {config_path}")
                    try:
                        with open(config_path, 'r', encoding='utf-8') as f:
                            config = yaml.safe_load(f)
                        
                        if config and isinstance(config, dict):
                            print(f"✅ DEBUG: Config caricato con keys: {list(config.keys())}")
                            
                            if 'paths' in config and 'database' in config['paths']:
                                db_path = config['paths']['database']
                                print(f"✅ DEBUG: Database path da config: {db_path}")
                                
                                # Verifica che il file DB esista
                                if Path(db_path).exists():
                                    print(f"✅ DEBUG: Database file ESISTE: {db_path}")
                                    
                                    # Crea DatabaseManager
                                    db_manager = DatabaseManager(db_path)
                                    print(f"✅ DEBUG: DatabaseManager creato con successo")
                                    return db_manager
                                else:
                                    print(f"❌ DEBUG: Database file NON ESISTE: {db_path}")
                                    return None
                            else:
                                print("❌ DEBUG: Config non ha paths.database")
                                if 'paths' in config:
                                    print(f"   Paths disponibili: {list(config['paths'].keys())}")
                        else:
                            print("❌ DEBUG: Config non valido o vuoto")
                        
                        break  # Config trovato (valido o no), non cercare oltre
                        
                    except Exception as e:
                        print(f"❌ DEBUG: Errore lettura {config_path}: {e}")
                else:
                    print(f"❌ DEBUG: NON ESISTE: {config_path}")
            
            if not config:
                print("❌ DEBUG: config_new.yaml NON TROVATO in nessun percorso")
                return None
            
            print("❌ DEBUG: config_new.yaml trovato ma configurazione database non valida")
            return None
                
        except ImportError:
            print("❌ DEBUG: DatabaseManager modulo non trovato")
            return None
        except Exception as e:
            ...
            for config_path in config_paths:
                ...
                if config_path.exists():
                    print(f"✅ DEBUG: config.yaml TROVATO: {config_path}")
                    try:
                        with open(config_path, 'r', encoding='utf-8') as f:
                            config = yaml.safe_load(f)
            ...
            if not config:
                print("❌ DEBUG: config.yaml NON TROVATO in nessun percorso")
                return None
            
            print("❌ DEBUG: config.yaml trovato ma configurazione database non valida")
            return None
    ...
    def _load_exif_data(self):
        ...
        try:
            ...
            for i, item in enumerate(self.items):
                ...
                try:
                    # Debug: verifica dati item
                    item_id = getattr(item, 'image_id', None) or item.image_data.get('id')
                    print(f"🔍 DEBUG CompleteExifDialog - Item ID: {item_id}")
                    print(f"🔍 DEBUG CompleteExifDialog - Item data keys: {list(item.image_data.keys())}")
                    
                    if not item_id:
                        print("❌ ERROR: Nessun ID trovato per l'item")
                        exif_json_str = None
                    else:
                        ...
                        if hasattr(self, 'parent') and self.parent and hasattr(self.parent, 'db_manager'):
                            ...
                            print("🔍 DEBUG: Usando db_manager dal parent")
                        # Metodo 2: Tramite gallery parent
                            ...
                            print("🔍 DEBUG: Usando db_manager dal parent.parent")
                        # Metodo 3: Import diretto corretto
                        else:
                            try:
                                from db_manager_new import DatabaseManager
                                db_conn = DatabaseManager('database/offgallery.sqlite')
                                print("🔍 DEBUG: Usando DatabaseManager da db_manager_new")
                            except:
                                try:
                                    import sqlite3
                                    db_conn = sqlite3.connect('database/offgallery.sqlite')
                                    print("🔍 DEBUG: Usando SQLite diretto")
                                except Exception as e:
                                    print(f"❌ DEBUG: Tutti i metodi DB falliti: {e}")
                                    exif_json_str = None
                        
                        if db_conn:
                            print(f"🔍 DEBUG: Eseguendo query per ID {item_id}")
                            ...
                            if result and result[0]:
                                ...
                                print(f"✅ DEBUG: EXIF JSON trovato - {len(exif_json_str)} caratteri")
                            else:
                                print("❌ DEBUG: Nessun EXIF JSON trovato nel database")
                                exif_json_str = None
                        else:
                            exif_json_str = None
                    
                except Exception as e:
                    print(f"❌ DEBUG: Errore query exif_json: {e}")
                    import traceback
                    traceback.print_exc()
                    exif_json_str = None
...
class LLMTagDialog(QDialog):
    ...
    def _validate_methods(self):
        """Debug: Valida che tutti i metodi necessari esistano"""
        required_methods = [
            'get_mode', 'get_tag_categories', 'get_config', 
            'get_selected_mode', 'get_categories', 'get_items',
            'get_num_images', 'get_full_config', 'get_description_style',
            'get_tag_style', 'get_language', 'get_detail_level', 
            'get_all_settings', 'get_hints'
        ]
        ...
        if missing:
            print(f"🚨 LLMTagDialog - Metodi mancanti: {missing}")
        else:
            print(f"✅ LLMTagDialog - Tutti i {len(required_methods)} metodi disponibili")
            
        print(f"📋 LLMTagDialog - Metodi esistenti: {len(existing)}")
        print(f"🛡️ LLMTagDialog - Safety net __getattr__ attivo per metodi get_* non implementati")
    ...
    def __getattr__(self, name):
        ...
        if name.startswith('get_'):
            print(f"⚠️ LLMTagDialog - Metodo {name} richiesto ma non implementato, restituisco default")
