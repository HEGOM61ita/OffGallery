...
class MainWindow(QMainWindow):
    ...
    def get_selected_gallery_items(self):
        """Test debug selected_items"""
        print("\n" + "="*50)
        print("🔍 DEBUG get_selected_gallery_items()")
        print("="*50)
    
        if hasattr(self.gallery_tab, 'selected_items'):
            selected = self.gallery_tab.selected_items
            print(f"✅ selected_items esiste: {type(selected)}")
            print(f"📊 Lunghezza: {len(selected)}")
        
            if len(selected) > 0:
                print("📋 Items selezionati:")
                for i, item in enumerate(selected):
                    filename = "Unknown"
                    if hasattr(item, 'image_data'):
                        filename = item.image_data.get('filename', 'Unknown')
                    print(f"   {i+1}. {filename} ({type(item).__name__})")
            else:
                print("⚠️  selected_items è vuoto!")
        
            print("="*50 + "\n")
            return selected
        else:
            print("❌ selected_items NON ESISTE!")
            print("="*50 + "\n")
            return []
    ...
    def update_database_manager(self, new_db_path=None):
        ...
        try:
            ...
            if self.db_manager:
                ...
                print("🔄 Database manager precedente chiuso")
            ...
            if new_db_path:
                ...
                print(f"✅ Database manager aggiornato: {new_db_path}")
        ...
        for tab_attr, tab_name in tab_list:
            if hasattr(self, tab_attr):
                ...
                if hasattr(tab, 'set_database_manager'):
                    ...
                    print(f"✅ Database propagato a {tab_name}")
    ...
    def closeEvent(self, event):
        ...
        if hasattr(self, 'db_manager') and self.db_manager:
            try:
                self.db_manager.close()
                print("✅ Database manager chiuso correttamente")
            except Exception as e:
