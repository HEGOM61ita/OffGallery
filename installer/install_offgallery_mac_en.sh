...
# Run with: bash install_offgallery_mac_en.sh
# ═══════════════════════════════════════════════════════════════════
...
# --- Scenario C: installation needed ---
    ...
    if ! bash "$MINICONDA_INSTALLER" -b -p "$MINICONDA_DIR"; then
        ...
        echo "     bash installer/install_offgallery_mac_en.sh"
        rm -f "$MINICONDA_INSTALLER"
        exit 1
    fi
...
# Install cmake via conda (required by llvmlite/numba to compile from source)
echo "   [2/3] Installing cmake (required by llvmlite)..."
...
echo "   approximately 8 GB of AI models. This is normal and happens"
echo "   only once:"
echo ""
echo "     - SigLIP so400m (semantic search):     ~1.8 GB"
