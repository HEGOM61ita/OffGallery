...
Nessun terminale, nessuna configurazione manuale. Installa automaticamente Miniconda, ambiente Python, librerie e modelli AI (~8 GB).
...
> **Metodo alternativo — script da terminale**: Se hai scaricato lo ZIP da GitHub e preferisci il metodo tradizionale, apri la cartella `installer\` e fai doppio click su **`INSTALLA_OffGallery.bat`**, poi segui le istruzioni a schermo (rispondi S/N).
...
OffGallery richiede versioni specifiche di alcune librerie per garantire la compatibilità con il modello SigLIP so400m. Se riscontri problemi con la ricerca semantica (score sempre vicini a zero) o errori di shape mismatch, esegui:
...
Il wizard crea automaticamente il collegamento **OffGallery** sul Desktop:
- **OffGallerySetup.exe** (metodo preferito): crea **OffGallery.lnk** (launcher principale) e **OffGallery Manager.lnk** (per aggiornamenti e manutenzione)
- **INSTALLA_OffGallery.bat** (metodo script): crea **OffGallery.lnk**

Fai **doppio click** sul collegamento **OffGallery** per avviare l'app.
...
|---------|-----|------------|
| **SigLIP** | Ricerca semantica multilingua | ~1.8 GB |
...
- Prova da terminale: `bash installer/offgallery_launcher_linux.sh`
