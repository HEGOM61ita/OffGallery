...
class EmbeddingGenerator:
    ...
    def _load_optimization_profiles(self) -> Dict:
        ...
        logger.info(f"✓ Profili ottimizzazione caricati: {list(final_profiles.keys())}")
    ...
    def _init_translator(self):
        ...
        try:
            ...
            if not it_en_installed:
                logger.info("Argos IT->EN non installato, tentativo download...")
                ...
                if frozen_repo:
                    try:
                        ...
                        logger.info(f"Argos: download da repo congelato {frozen_repo}/{argos_subfolder}...")
                        ...
                        downloaded = True
                        logger.info(f"[OK] Argos IT->EN installato da {frozen_repo}")
                ...
                if not downloaded:
                    try:
                        logger.info("Argos: download da server ufficiale...")
    ...
    def _init_clip(self):
        ...
        try:
            ...
            if not loaded:
                logger.info(f"CLIP: caricamento da {fallback_model}...")
                ...
                logger.info(f"[OK] CLIP caricato da {fallback_model} (fallback)")
    ...
    def _init_dinov2(self):
        ...
        try:
            ...
            # Fallback repo ufficiale
            if not loaded:
                logger.info(f"DINOv2: caricamento da {fallback_model}...")
                ...
                logger.info(f"[OK] DINOv2 caricato da {fallback_model} (fallback)")
    ...
    def _init_aesthetic(self):
        ...
        try:
            ...
            if local_exists:
                try:
                    logger.info("Aesthetic: caricamento da cache locale...")
                    ...
                    logger.info("[OK] Aesthetic caricato da cache locale")
                except Exception as e:
            ...
            if clip_model is None:
                logger.info(f"Aesthetic: caricamento da {fallback_model}...")
                ...
                clip_model.save_pretrained(str(aesthetic_dir))
                self.aesthetic_processor.save_pretrained(str(aesthetic_dir))
                logger.info(f"[OK] Aesthetic caricato da {fallback_model} (fallback) e salvato in locale")
    ...
    def _init_brisque(self):
        """Inizializza BRISQUE con debug esteso"""
        try:
            import cv2
            app_dir = get_app_dir()

            print(f"🔧 DEBUG BRISQUE: app_dir = {app_dir}")
            print(f"🔧 DEBUG BRISQUE: app_dir assoluto = {app_dir.absolute()}")
            ...
            print(f"🔧 DEBUG BRISQUE: model_path = {self.brisque_model_path}")
            print(f"🔧 DEBUG BRISQUE: range_path = {self.brisque_range_path}")
            print(f"🔧 DEBUG BRISQUE: model exists = {self.brisque_model_path.exists()}")
            print(f"🔧 DEBUG BRISQUE: range exists = {self.brisque_range_path.exists()}")
            
            # Test lettura file
            if self.brisque_model_path.exists():
                try:
                    with open(self.brisque_model_path, 'r') as f:
                        content = f.read(100)  # Prime 100 char
                    print(f"🔧 DEBUG BRISQUE: model file leggibile, inizio: {repr(content[:50])}")
                except Exception as e:
                    print(f"🔧 DEBUG BRISQUE: ERRORE lettura model file: {e}")
            
            if self.brisque_model_path.exists() and self.brisque_range_path.exists():
                # Test caricamento OpenCV
                try:
                    test_brisque = cv2.quality.QualityBRISQUE_create(str(self.brisque_model_path), str(self.brisque_range_path))
                    self.brisque_available = True
                    print(f"🔧 DEBUG BRISQUE: OpenCV caricamento SUCCESSO")
                except Exception as e:
                    print(f"🔧 DEBUG BRISQUE: ERRORE OpenCV: {e}")
                    self.brisque_available = False
            else:
                print(f"🔧 DEBUG BRISQUE: File mancanti, provo fallback...")
                try:
                    _ = cv2.quality.QualityBRISQUE_create()
                    self.brisque_available = True
                    self.brisque_model_path = None
                    self.brisque_range_path = None
                    print(f"🔧 DEBUG BRISQUE: Fallback OpenCV default SUCCESSO")
                except Exception as e:
                    print(f"🔧 DEBUG BRISQUE: ERRORE anche fallback: {e}")
                    self.brisque_available = False
            
            logger.info(f"[OK] BRISQUE status: {self.brisque_available}")
    ...
    def _init_bioclip(self):
        ...
        try:
            ...
            logger.info("Caricamento BioCLIP TreeOfLife classifier...")
                ...
                if frozen_repo:
                    try:
                        ...
                        if not bioclip_local_ok:
                            logger.info(f"BioCLIP: download da {frozen_repo}/{bioclip_subfolder}...")
                        ...
                        if not treeoflife_local_ok:
                            logger.info(f"TreeOfLife: download da {frozen_repo}/{treeoflife_subfolder}/embeddings...")
            ...
            if bioclip_local_ok and treeoflife_local_ok:
                try:
                    logger.info("BioCLIP: caricamento da cache locale...")
                    ...
                    logger.info("[OK] BioCLIP caricato da cache locale")
                    return True
            ...
            logger.info("BioCLIP: caricamento da repo ufficiale (fallback)...")
            self.bioclip_classifier = TreeOfLifeClassifier(device=self.device)
            logger.info("[OK] BioCLIP TreeOfLife inizializzato (fallback)")
            return True
