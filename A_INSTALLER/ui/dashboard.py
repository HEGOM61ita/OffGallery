...
class DashboardPage(tk.Frame):
    ...
    def _build(self):
        ...
        # copia a cui punta il collegamento "OffGallery Manager". Chi lancia il
        # setup nuovo dalla cartella Download non vedrebbe altrimenti nulla, e
        # continuerebbe ad aprire dal collegamento quella di mesi prima.
        self._copia_bar = tk.Frame(self, bg="#fff3cd")
        self._copia_lbl = tk.Label(
            self._copia_bar, bg="#fff3cd", fg="#664d03",
            font=("Segoe UI", 9), justify="left", anchor="w", wraplength=620)
        self._copia_lbl.pack(side="left", padx=(16, 8), pady=6)
        self._copia_btn = ttk.Button(self._copia_bar, text="Aggiorna il collegamento",
        ...
        # La larghezza NON e' piu' un numero fisso. Fissarla in pixel mentre le
        # righe dentro si dimensionano in caratteri (width=22, 16, 11) e' la
        # causa dei pulsanti tagliati: quanto spazio chieda davvero una riga
        # dipende dal font di sistema e da come Windows lo arrotonda alla
        # scalatura in uso, e nessuna costante indovina tutti i casi. Il 440
        # precedente era gia' sotto di qualche pixel al 100% (segnalazione
        # 2026-08-28, poi 2026-08-31 su schermo al 150%).
        # Ora la colonna viene misurata sul contenuto reale in
        # _adatta_colonna_sinistra(), chiamata a costruzione finita.
    ...
    def _adatta_colonna_sinistra(self):
        ...
        quanti pixel occupino dipende dal font di sistema e dalla scalatura
        dello schermo, che Windows arrotonda in modo non proporzionale (al
        150% un 9pt puo' diventare 14pt, cioe' 1,55x e non 1,50x). Misurarle
        e' l'unico modo per non tagliare mai il pulsante di destra, a
        qualunque scalatura e con qualunque font.
        """
        try:
    ...
    def _check_copia_manager(self):
        """Verifica in un thread se la copia nel collegamento e' rimasta indietro."""
        sm = self.app.state
    ...
    def _show_copia_bar(self, nuova: Optional[str]):
        ...
        self._copia_lbl.configure(
            text=("Il collegamento \u00abOffGallery Manager\u00bb apre una versione "
                  f"vecchia. Aggiornalo a {nuova}: l'applicazione non viene "
                  "toccata."))
        self._copia_bar.pack(fill="x", after=self._header)

    def _aggiorna_copia_manager(self):
        """Scarica il Manager della release e sostituisce la copia installata."""
        sm = self.app.state
        if not sm or not getattr(sm, "install_path", ""):
            return
        install_path = sm.install_path
    ...
    def _copia_esito(self, riuscito: bool, testo: str):
        if riuscito:
            self._copia_lbl.configure(
                text=(f"Fatto. Il collegamento \u00abOffGallery Manager\u00bb ora apre "
                      f"la versione {testo}."))
