"""
Dashboard di manutenzione — Linux.
Frame puro, nessuna finestra propria.
...
class _Tooltip:
    ...
    def _show(self):
        ...
        tk.Label(
            ...
            font=("sans-serif", 9), bg="#1a1a2e", fg="#e8e8f0",
...
TOOLTIP_TEXT: dict[str, str] = {
    # SISTEMA
    "exiftool": (
        "ExifTool — Lettore metadati fotografici.  (sistema)\n\n"
        "Legge i dati EXIF/IPTC/XMP dalle immagini: data di scatto, "
        "fotocamera, obiettivo, impostazioni, GPS.\n\n"
        "Su Linux viene installato tramite il package manager di sistema "
        "(apt/dnf/pacman). Richiede una sola volta la password sudo."
    ),
    # AMBIENTE
    "core": (
    ...
    "packages": (
        ...
        "Include automaticamente la variante GPU (CUDA/ROCm) se rilevata, "
        "altrimenti CPU-only. "
    ...
    "ollama": (
        ...
        "Su Linux viene installato tramite lo script ufficiale ollama.com.\n\n"
    ...
    "lmstudio": (
        ...
        "Su Linux viene installato come AppImage in ~/.local/bin/.\n\n"
        "Non installato ora? Puoi aggiungerlo in qualsiasi momento "
        "cliccando [Installa] da questa schermata."
    ),
}
...
class DashboardPage(tk.Frame):
    ...
    def _build(self):
        ...
        ttk.Button(btn_frame, text="↻  Aggiorna stato",
                   width=22, command=self._refresh_all).pack(side="right", ipadx=10, ipady=4)
    ...
    def _refresh_all(self):
        ...
        # ExifTool
        et_ok = exiftool_installed()
        self._set_row("exiftool",
                      "done" if et_ok else "not_installed",
                      "installato" if et_ok else "—",
                      action_label=None if et_ok else "Installa",
                      action=None if et_ok else self._action_install_exiftool)
    ...
    def _action_install_exiftool(self):
        def _do():
            try:
                self._set_row("exiftool", "in_progress", "Installazione (sudo)...")
                self._panel.set_step_indeterminate("Installazione ExifTool tramite package manager...")
                ok = install_exiftool(log_cb=self._panel.log)
                self._panel.set_step_done("ExifTool installato" if ok else "Installazione manuale richiesta")
                if ok:
                    self.app.state.mark_done("exiftool_linux")
                else:
                    self.app.state.mark_error("exiftool_linux", "fallito")
...
class _ComponentRow(tk.Frame):
    ...
    def __init__(self, parent, key: str, label: str, dashboard: DashboardPage,
        ...
        self._icon_lbl = tk.Label(self, textvariable=self._icon_var,
                                  font=("sans-serif", 11), bg=BG, width=3)
