"""
Pagine del wizard di prima installazione — Linux.
...
# Font cross-platform: tkinter usa il font di sistema su Linux
    ...
    def _build(self):
        ...
        tk.Label(info_box,
                 text="ℹ️  La prima installazione scarica modelli AI e librerie (~8–14 GB).\n"
                      ...
                      "    La fase ExifTool richiede la password sudo una sola volta.",
        ...
        for value, label, desc in [
            ...
            tk.Radiobutton(row, variable=self._profile_var, value=value,
                           text=label, font=("sans-serif", 10, "bold"),
...
class PreflightPage(tk.Frame):
    ...
    def _show_results(self, report):
        ...
        for r in report.results:
            ...
            tk.Label(row, text=icon, font=("sans-serif", 11),
                     bg=BG, width=3).pack(side="left")
            tk.Label(row, text=r.name, font=("sans-serif", 10, "bold"),
...
class PathPage(tk.Frame):
    ...
    def _update_disk_label(self):
        ...
        try:
            ...
            free  = shutil.disk_usage(path if os.path.exists(path) else os.path.dirname(path) or "/").free
...
class InstallPage(tk.Frame):
    ...
    def _run_install(self):
        ...
        self._logger.section("Avvio installazione Linux")

        try:
            # ExifTool (tramite package manager di sistema, richiede sudo)
            if not sm.is_done("exiftool_linux"):
                self._step("ExifTool", "in_progress")
                self._panel.set_step_indeterminate("Installazione ExifTool (richiede sudo)...")
                ok = install_exiftool(log_cb=self._log)
                if ok:
                    sm.mark_done("exiftool_linux")
                    self._step("ExifTool", "done", "installato")
                else:
                    sm.mark_error("exiftool_linux", "installazione manuale richiesta")
                    self._step("ExifTool", "error", "installa manualmente")
                self._results["ExifTool"] = ok
            else:
                self._step("ExifTool", "done", "(già presente)")
                self._results["ExifTool"] = True
...
class DonePage(tk.Frame):
    ...
    def _build(self):
        ...
        tk.Label(frame, text="🎉", font=("sans-serif", 48), bg=BG).pack()
        ...
        tk.Label(frame,
                 text="OffGallery è pronto all'uso sul tuo computer.\n"
                      "Trovi il collegamento nel menu applicazioni e sul Desktop.",
        ...
        tk.Label(frame, textvariable=self._log_path_var,
                 font=("sans-serif", 8), bg=BG, fg="#777",
