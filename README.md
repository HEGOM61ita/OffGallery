...
| Data | Cosa | Note |
|------|------|------|
| 3 set 2026 | **Correzioni e stabilità (v1.0.43 → v1.0.50)** | Ricerca: risolto uno sfarfallio dell'interfaccia su archivi grandi e il caso in cui le foto ruotate tornavano dritte nei risultati. Gallery: selezione multipla con Shift+click, cancellazione più affidabile. Elaborazione: descrizioni corrette sui RAW scattati in bianco e nero. Installer/Manager: pulsanti non più tagliati a nessuna risoluzione, la cartella di installazione scelta manualmente viene ora ricordata tra un avvio e l'altro. |
...
| Date | What | Notes |
|------|------|-------|
| 3 Sep 2026 | **Fixes and stability (v1.0.43 → v1.0.50)** | Search: fixed an interface flicker on large libraries and a case where rotated photos showed upright again in results. Gallery: multi-select with Shift+click, more reliable deletion. Processing: fixed descriptions on RAW files shot in black & white. Installer/Manager: buttons no longer clipped at any resolution, custom install folder is now remembered between launches. |
| 11 Jul 2026 | **Plugins free for everyone** | All plugins (BioNomen, GeoNames, GeoSpecies, NaturArea, Weather, Ollama, LM Studio, Prompt Context) are now freely downloadable, no email request needed anymore. **Windows and Linux**: install them from the graphical installer, «Plugins» section. **macOS**: download the zip from the [plugins page](https://offgallery.app/#beta) and extract into `plugins/`. Zips contain only code; data (maps, species databases) is downloaded at runtime. Includes a BioNomen fix: common names now always match the configured output language. |
